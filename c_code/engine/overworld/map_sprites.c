...
// ld de, wSpritePlayerStateData2PictureID
    de = wSpritePlayerStateData2PictureID;
...
// ld hl, wSpritePlayerStateData2PictureID
    hl = wSpritePlayerStateData2PictureID;
// xor a
    a = 0;
// ldh [hFourTileSpriteCount], a
    hFourTileSpriteCount = a;
...
// ld a, [hli] ; a = [x#SPRITESTATEDATA2_PICTUREID]
    a = hli] ; a = [x#SPRITESTATEDATA2_PICTUREID;
...
// ld hl, wSprite01StateData2ImageBaseOffset
    hl = wSprite01StateData2ImageBaseOffset;
// .loadTilePatternLoop
// ld de, wSprite01StateData2PictureID
    de = wSprite01StateData2PictureID;
...
// ld de, wSpritePlayerStateData2ImageBaseOffset
    de = wSpritePlayerStateData2ImageBaseOffset;
// ld b, 1
...
// ld a, b ; a = current sprite picture ID
    a = b ; a = current sprite picture ID;
// dec a
// add a
// add a
// push bc
...
// ld hl, vNPCSprites ; VRAM base address
    hl = vNPCSprites ; VRAM base address;
// ld bc, 12 tiles ; number of bytes per VRAM slot
    bc = 12 tiles ; number of bytes per VRAM slot;
// ldh a, [hVRAMSlot]
    a = hVRAMSlot;
...
// ld hl, vSprites tile $7c ; address for second 4-tile sprite
    hl = vSprites tile $7c ; address for second 4-tile sprite;
// ldh a, [hFourTileSpriteCount]
    a = hFourTileSpriteCount;
// and a
...
// ld hl, vSprites tile $78 ; address for first 4-tile sprite
    hl = vSprites tile $78 ; address for first 4-tile sprite;
// inc a
// ldh [hFourTileSpriteCount], a
    hFourTileSpriteCount = a;
...
// call FarCopyData2 ; load tile pattern data for sprite when standing still
    FarCopyData2 ; load tile pattern data for sprite when standing still();
// .skipFirstLoad
// pop de
// pop hl
// ldh a, [hVRAMSlot]
...
// call FarCopyData2 ; load tile pattern data for sprite when walking
    FarCopyData2 ; load tile pattern data for sprite when walking();
// jr .skipSecondLoad
...
// call CopyVideoData ; load tile pattern data for sprite when walking
    CopyVideoData ; load tile pattern data for sprite when walking();
// .skipSecondLoad
// pop hl
// pop bc
// jr .nextSpriteSlot
...
// ld a, [de] ; a = [y#SPRITESTATEDATA2_IMAGEBASEOFFSET]
    a = de] ; a = [y#SPRITESTATEDATA2_IMAGEBASEOFFSET;
...
// call nc, GetSplitMapSpriteSetID ; if so, choose the appropriate one
    nc, GetSplitMapSpriteSetID ; if so, choose the appropriate one();
...
// ld a, SPRITE_SET_LENGTH ; 11 sprites in sprite set
    a = SPRITE_SET_LENGTH ; 11 sprites in sprite set;
// ld [wNumSprites], a
    wNumSprites = a;
...
// ld a, [hl] ; [x#SPRITESTATEDATA1_PICTUREID] (zero if sprite slot is not used)
    a = hl] ; [x#SPRITESTATEDATA1_PICTUREID;
...
// ld a, SPRITESTATEDATA2_IMAGEBASEOFFSET - SPRITESTATEDATA1_PICTUREID
    a = SPRITESTATEDATA2_IMAGEBASEOFFSET - SPRITESTATEDATA1_PICTUREID;
// add l
// ld l, a
// ld a, c ; a = VRAM slot (zero if sprite slot is not used)
    a = c ; a = VRAM slot (zero if sprite slot is not used);
// ld [hl], a ; [x#SPRITESTATEDATA2_IMAGEBASEOFFSET]
    *hl = a;
// pop hl
// ld a, SPRITESTATEDATA1_LENGTH
